"""
LLaMA 3 8B Instruct model wrapper.

Meta's flagship open-source LLM with strong multilingual capabilities.
Uses standard HuggingFace transformers API with BitsAndBytes quantization.
"""
...
class LLaMAWrapper(BaseModel):
    """Wrapper for Meta LLaMA 3 8B Instruct."""
    ...
    def _load_model(self) -> None:
        ...
        self.model.eval()
        logger.info(f"LLaMA 3 loaded with {sum(p.numel() for p in self.model.parameters()):,} parameters")
    ...
    def generate(
        ...
        **kwargs,
        ...
        return GenerationOutput(
            ...
            metadata={"temperature": temperature, "top_p": top_p, "max_new_tokens": max_new_tokens},
        )
    ...
    def classify(
        ...
        **kwargs,
        ...
        """Classify text using LLaMA 3 via generation."""
        if not self._is_loaded:
            self.load()
        ...
        for text in texts:
            prompt = (
                ...
                f"Text: {text}\n\n"
                f"Classification (output ONLY the category name):"
            )
